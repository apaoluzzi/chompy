...
def polyline(points):
    ...
    """    
    # needed to correctly manage cycles (closed polylines) -------
    ptk,verts = remap(points) 
    
    cells = [AA(ptk)([k,k+1]) for k in range(len(points)-1)]
...
def trianglefan (points):
    """
    Primitive constructor of a fan of triangles.

    To produce the triangle fan generated by the ordered set of `points`
    and by its centroid as pivot.
    ...
    n = len(points)
    cells = [edge + [n-1] for edge in edges.cells[1]]
    return SimplicialComplex(points,cells)
