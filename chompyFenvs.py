...
def s(coords):
    """
    Scaling function (3rd order: to apply 3 times to return an object.)

    Return either a partial function or a new cellular complex,
    ...
    def scale0(params):
        def scale1(obj):
            ...
            for k in range(len(params)):
                vector[coords[k]] = params[k]
            return obj.scale(vector)
        return scale1
    return scale0
...
def t(coords):
    """
    Translation function (3rd order: to apply 3 times to return an object.)

    Return either a partial function or a new cellular complex,
    without changing the input type.
    """
    def translate0(params):
        def translate1(obj):
            vector = (obj.rn)*[0.0]
            for k in range(len(params)):
                vector[coords[k]] = params[k]
...
def r(coords):
    """
    Rotation function (3rd order: to apply 3 times to return an object.)
    ...
    def rotate0(angle):
        def rotate1(obj):
            return obj.rotate(axis1,axis2, angle)
        return rotate1
    return rotate0
